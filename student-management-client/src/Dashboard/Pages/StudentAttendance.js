...
export default function StudentAttendance() {
  ...
  //search
  async function GetCourseSearched() {
    try {
      const url = `${base_url}attendance/search?course=${course}`;
      const response = await fetch(url);
      const getdata = await response.json();
      setStudentList(getdata.course.students);
      setCourseDetails(getdata.course);
                ...
                >
                  <div>
                    ...
                    <div style={{ display: "flex", gap: "1rem" }}>
                      {" "}
                      <b>Department</b>
                      {/* <t />:<p>{courseDetails.department.title}</p> */}
                    </div>
                    <div style={{ display: "flex", gap: "1rem" }}>
                      {" "}
                      <b>Year of Study</b>:<p>{courseDetails.year}</p>
                    </div>
                    <div style={{ display: "flex", gap: "1rem" }}>
                      {" "}
                  ...
                  <div>
                    <div style={{ display: "flex", gap: "1rem" }}>
                      <b>Course Code</b>:<p>{courseDetails.code}</p>
                    </div>
                    <div style={{ display: "flex", gap: "1rem" }}>
                      <b>Course Title</b>:<p>{courseDetails.title}</p>
                    </div>
                    <div style={{ display: "flex", gap: "1rem" }}>
                      {" "}
